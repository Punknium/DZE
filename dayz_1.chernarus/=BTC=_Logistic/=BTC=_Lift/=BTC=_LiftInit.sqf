...
while {true} do 
{
	...
	if (!(vehicle player == player) && driver vehicle player == player) then {
		...
		if (BTC_Hud_Cond) then {
			BTC_liftHudId = _chopper addAction [("<t color=""#ED2744"">" + ("Lift Hud Off") + "</t>"),"=BTC=_Logistic\=BTC=_Lift\=BTC=_Hud.sqf", "", 0, false, false];
		} else {
			BTC_liftHudId = _chopper addAction [("<t color=""#ED2744"">" + ("Lift Hud On") + "</t>"),"=BTC=_Logistic\=BTC=_Lift\=BTC=_Hud.sqf", "", 0, false, false];
		};
	};
	...
	while {!(vehicle player == player) && (driver vehicle player == player)} do
	{
		...
		if (!isNull _cargo && BTC_lifted == 0 && _can_lift && (!locked _cargo || (getPlayerUID player) in ["147505542"])) then {
