...
if ((getPlayerUID player) in ["147505542"]) then {
	if (s_player_pzombiesvision < 0) then {
		s_player_pzombiesvision = player addAction [localize "STR_EPOCH_ACTIONS_NIGHTVIS", "custom\vision.sqf", [], 4, false, true, "", ""];
	};
	if (s_admin_debug < 0) then {
		if(a_d) then {
			s_admin_debug = player addAction ["Admin Debug Off", "custom\admin_actions.sqf", ["ad0"], 3, false, true, "", ""];
		} else {
			s_admin_debug = player addAction ["Admin Debug On", "custom\admin_actions.sqf", ["ad1"], 3, false, true, "", ""];
		};
	};
	if (s_admin_startmission < 0) then {
		s_admin_startmission = player addAction ["Admin Start Mission", "custom\admin_actions.sqf", ["startmission", "zombies"], 3, false, true, "", ""];
	};
	if (s_admin_boom < 0) then {
		s_admin_boom = player addAction ["Admin Boom", "custom\admin_actions.sqf", ["boom"], 3, false, true, "", ""];
	};
};
