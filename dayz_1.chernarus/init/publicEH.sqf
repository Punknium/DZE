...
if (isServer) then {
	...
	"PVDZE_getFPS"			 addPublicVariableEventHandler {(_this select 1) call sendFPS};
	"PVDZE_startMission"	 addPublicVariableEventHandler {(_this select 1) call server_startmission};
};
...
if (!isDedicated) then {
	...
	"PVDZE_donatorList"		addPublicVariableEventHandler {donatorList = (_this select 1) select 0; donatorLoaded = true};
	"PVDZE_spawnZed"		addPublicVariableEventHandler {(_this select 1) call zedSpawn};
	"PVDZE_recFPS"			addPublicVariableEventHandler {serverFPS = floor (_this select 1)};
};
