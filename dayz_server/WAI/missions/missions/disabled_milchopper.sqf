...
[[_position select 0, (_position select 1) - 300, 0],                  //position
8,						  //Number Of units
1,					      //Skill level 0-1. Has no effect if using custom skills
"Random",			      //Primary gun set number. "Random" for random weapon set.
4,						  //Number of magazines
...
//Turrets
[[[(_position select 0) + 10, (_position select 1), 0]], //position(s) (can be multiple).
"M2StaticMG",             //Classname of turret
0.5,					  //Skill level 0-1. Has no effect if using custom skills
"Bandit2_DZ",				          //Skin "" for random or classname here.
1,						  //Primary gun set number. "Random" for random weapon set. (not needed if ai_static_useweapon = False)
...
[nil,nil,rTitleText,"A redneck bandit helicopter is taking off with a crate of sniper guns! Save the cargo and keep the guns for yourself.", "PLAIN",10] call RE;
...
if (_playerPresent) then {
	...
	[nil,nil,rTitleText,"Redneck survivors have secured the armed chopper!", "PLAIN",10] call RE;
} else {
	...
	[nil,nil,rTitleText,"Redneck survivors did not secure the armed chopper in time!", "PLAIN",10] call RE;
};
